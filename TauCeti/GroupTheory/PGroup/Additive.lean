...
@[simp]
theorem subquotientEquivZModOfEqSupZmultiples_apply_generator {N K : AddSubgroup M} {x : M}
    (hgen : K = N ⊔ AddSubgroup.zmultiples x) (hx : x ∉ N) (hpx : p • x ∈ N)
    (hxK : x ∈ K) :
    subquotientEquivZModOfEqSupZmultiples hgen hx hpx
      ((⟨x, hxK⟩ : K) : K ⧸ N.addSubgroupOf K) = 1 :=
  zmodAddEquivOfGenerator_symm_apply_generator _ _
