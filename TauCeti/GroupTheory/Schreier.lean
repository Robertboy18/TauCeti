...
/-!
# Schreier transversals and the sharp Schreier index formula
...
private theorem IsPartialSchreierTransversal.card_le_index [H.FiniteIndex] {R : Finset G}
    (hR : IsPartialSchreierTransversal H S R) : R.card ≤ H.index := by
...
/-- A partial Schreier transversal that misses a coset extends by one element. -/
private theorem IsPartialSchreierTransversal.exists_insert [DecidableEq G] (hS : closure S = ⊤)
    {R : Finset G} (hR : IsPartialSchreierTransversal H S R) {g : G}
    (hg : ∀ r ∈ R, g * r⁻¹ ∉ H) : ∃ x ∉ R, IsPartialSchreierTransversal H S (insert x R) := by
...
/-- A partial Schreier transversal representing every coset is a Schreier transversal. -/
private theorem IsPartialSchreierTransversal.isSchreierTransversal {R : Finset G}
    (hR : IsPartialSchreierTransversal H S R) (hcomp : ∀ g : G, ∃ r ∈ R, g * r⁻¹ ∈ H) :
    H.IsSchreierTransversal S R where
  isComplement := by
...
/-- **Existence of Schreier transversals.** A subgroup of finite index has a finite Schreier
transversal with respect to every generating set of the ambient group. -/
theorem _root_.Subgroup.exists_finset_isSchreierTransversal (H : Subgroup G) [H.FiniteIndex]
    (hS : closure S = ⊤) : ∃ R : Finset G, H.IsSchreierTransversal S R := by
  classical
  -- Enlarge a partial transversal one coset at a time; `m` bounds the number of missing cosets.
  suffices h : ∀ (m : ℕ) (R : Finset G), IsPartialSchreierTransversal H S R →
      H.index ≤ R.card + m → ∃ R : Finset G, H.IsSchreierTransversal S R from
  ...
  | zero =>
    ...
    · exact ⟨R, hR.isSchreierTransversal hcomp⟩
    · push Not at hcomp
      obtain ⟨g, hg⟩ := hcomp
