...
* `TauCeti.commutatorElement_pow_left_of_central`, `TauCeti.commutatorElement_pow_right_of_central`:
  the collection formulas `⁅a ^ n, b⁆ = ⁅a, b⁆ ^ n * ⁅a, ⁅a, b⁆⁆ ^ (n choose 2)` and
  `⁅a, b ^ n⁆ = ⁅a, b⁆ ^ n * ⁅b, ⁅a, b⁆⁆ ^ (n choose 2)` when the iterated commutator is central.
...
/-- **Collection of a power in the left input**: when the iterated commutator `⁅a, ⁅a, b⁆⁆` is
central, `⁅a ^ n, b⁆ = ⁅a, b⁆ ^ n * ⁅a, ⁅a, b⁆⁆ ^ (n choose 2)`. -/
theorem commutatorElement_pow_left_of_central {a b : G}
    (h : ∀ g : G, Commute g ⁅a, ⁅a, b⁆⁆) (n : ℕ) :
    ⁅a ^ n, b⁆ = ⁅a, b⁆ ^ n * ⁅a, ⁅a, b⁆⁆ ^ n.choose 2 := by
  -- Write `a = c * t` with `c = ⁅a, b⁆` and `t = b * a * b⁻¹`, so that `⁅t, c⁆ = ⁅a, ⁅a, b⁆⁆` is
  -- central and the binomial formula applies to `a ^ n = (c * t) ^ n`.
  let c := ⁅a, b⁆
  let t := b * a * b⁻¹
  let d := ⁅a, c⁆
...
/-- **Collection of a power in the right input**: when the iterated commutator `⁅b, ⁅a, b⁆⁆` is
central, `⁅a, b ^ n⁆ = ⁅a, b⁆ ^ n * ⁅b, ⁅a, b⁆⁆ ^ (n choose 2)`. -/
theorem commutatorElement_pow_right_of_central {a b : G}
