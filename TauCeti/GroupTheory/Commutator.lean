...
theorem _root_.Commute.mul_pow_eq_pow_mul_pow_mul_commutatorElement_pow_choose_two {a b : G}
  ...
  | zero => simp
  | succ n ih =>
    have hA : b ^ n * a = ⁅b, a⁆ ^ n * a * b ^ n := by
      rw [hb.commutatorElement_pow_left, commutatorElement_def]; group
    have h2 : (n + 1).choose 2 = n + n.choose 2 := by
      simpa using Nat.choose_succ_succ' n 1
    calc (a * b) ^ (n + 1)
        = a ^ n * (b ^ n * a) * b * ⁅b, a⁆ ^ n.choose 2 := by
          rw [pow_succ, ih, mul_assoc (a ^ n * b ^ n),
            ((ha.pow_right _).mul_left (hb.pow_right _)).symm.eq]
          simp only [mul_assoc]
      _ = a ^ n * (⁅b, a⁆ ^ n * (a * b ^ n * b)) * ⁅b, a⁆ ^ n.choose 2 := by
          rw [hA]; simp only [mul_assoc]
      _ = a ^ n * (a * b ^ n * b * ⁅b, a⁆ ^ n) * ⁅b, a⁆ ^ n.choose 2 := by
          rw [(((ha.pow_right n).symm.mul_right ((hb.pow_right n).pow_left n).symm).mul_right
            (hb.pow_right n).symm).eq]
      _ = a ^ (n + 1) * b ^ (n + 1) * ⁅b, a⁆ ^ (n + 1).choose 2 := by
          rw [h2, pow_succ a, pow_succ b, pow_add]; simp only [mul_assoc]
