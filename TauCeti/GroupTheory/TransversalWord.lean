...
`TauCeti.transversalDiff_mul_lWord`), and computes the word on the elements of a subgroup of index
two at the two-element transversal `{1, s}` (`TauCeti.indexTwoTransversal`), where it is the
identity at the trivial coset and conjugation by `s` at the other. Continuity of `γ ↦ ℓᵗ_u(γ)` for
...
(`TauCeti.indexTwoTransversal_mk`), the one on which the index-two corestriction formulas are
computed. -/
...
theorem indexTwoTransversal_mk_one (s : G) :
    indexTwoTransversal U s (QuotientGroup.mk 1) = 1 :=
  ite_eq_left rfl
...
theorem indexTwoTransversal_of_ne (s : G) {u : G ⧸ U} (hu : u ≠ QuotientGroup.mk 1) :
    indexTwoTransversal U s u = s :=
  ite_eq_right hu

/-- For a subgroup of index two and `s ∉ U`, `indexTwoTransversal U s` is a transversal. -/
theorem indexTwoTransversal_mk (hU : U.index = 2) {s : G} (hs : s ∉ U) (u : G ⧸ U) :
    (QuotientGroup.mk (indexTwoTransversal U s u) : G ⧸ U) = u := by
  rcases eq_mk_one_or_eq_mk_of_index_two hU hs u with rfl | rfl
  · rw [indexTwoTransversal_mk_one]
  · rw [indexTwoTransversal_of_ne s (mk_ne_mk_one_of_notMem hs)]

/-- At the trivial coset, the transversal word of `indexTwoTransversal U s` on an element of `U`
is that element. -/
theorem lWord_indexTwoTransversal_mk_one_of_mem (s : G) {γ : G} (hγ : γ ∈ U) :
    lWord U (indexTwoTransversal U s) (QuotientGroup.mk 1) γ = γ := by
  rw [lWord_mk_one_of_mem U _ hγ, indexTwoTransversal_mk_one, inv_one, one_mul, mul_one]

/-- At the coset of `s`, the transversal word of `indexTwoTransversal U s` on an element `γ` of a
subgroup `U` of index two is the conjugate `s⁻¹ * γ * s`. -/
theorem lWord_indexTwoTransversal_mk_of_mem (hU : U.index = 2) {s : G} (hs : s ∉ U) {γ : G}
    (hγ : γ ∈ U) :
    lWord U (indexTwoTransversal U s) (QuotientGroup.mk s) γ = s⁻¹ * γ * s := by
  ...
  rw [lWord_def, hcoset, indexTwoTransversal_of_ne s (mk_ne_mk_one_of_notMem hs)]
