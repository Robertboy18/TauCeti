...
`P.restrict S ⟶ P'.restrict S'` compatible with these inclusions.

The main result is that `P.restrictι S` induces isomorphisms on relative homology as soon as the
inclusions of `S` and of its preimage are quasi-isomorphisms on chains
(`SSetPair.isIso_homologyMap_restrictι`).  This is the algebraic step reducing excision for
singular homology to the small-chain theorem: restricting the singular pair of a topological pair
to the simplices subordinate to an open cover does not change relative homology.
...
lemma isIso_homologyMap_restrictι (R : A)
  ...
  have : QuasiIso (SSet.chainComplexMap (P.restrictι S).left R) := h₁
  have : QuasiIso (SSet.chainComplexMap (P.restrictι S).right R) := h₂
