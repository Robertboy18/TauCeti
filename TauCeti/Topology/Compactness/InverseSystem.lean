...
In other words the induced map on sequential inverse limits is surjective; no surjectivity of the
transition maps `α k` is needed, because compactness supplies it.

The proof applies `exists_forall_map_succ_eq_of_compact_t2` to the tower of fibres
`g k ⁻¹' {b k}`, which are nonempty closed subsets of the compact Hausdorff spaces `A k`. -/
