...
the `p`-power classes `π ḡ` and the brackets `[ḡ, h̄]` for `g, h ∈ s`. The argument: `λ_1` is the
closure of the subgroup generated by the `p`-th powers and the commutators, the preimage in `G`
of a subspace of `gr_1(G)` is a union of cosets of `λ_2`, hence open and closed, and `π` and the
bracket carry the span of the degree-zero classes into the span of the generators, by
bilinearity of the bracket and the degree-zero formula
`π (x + y) = π x + π y + (p choose 2) • [y, x]`.

For a linearly ordered index type the generators are packaged as `TauCeti.degreeOneFamily`: the
`p`-power classes `π ȳ_i` together with the brackets `[ȳ_i, ȳ_j]` for `i < j`. For a free pro-`p`
...
`p`-power classes `π ȳ_i` and the brackets `[ȳ_i, ȳ_j]` for `i < j`, in `gr_1(G)`. When `y`
