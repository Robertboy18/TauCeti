...
* `Subgroup.denseRange_subgroupOf_codRestrict`: a dense subgroup meets an open subgroup `U` in a
  subgroup dense in `U`.
...
range. -/
theorem _root_.Subgroup.denseRange_subgroupOf_codRestrict (hD : Dense (D : Set G))
    (hU : IsOpen (U : Set G)) :
...
finitely generated: a finite topological generating set `s` of `G` generates a dense subgroup `D`,
the subgroup `U ⊓ D` has finite index in `D` and so is finitely generated by Schreier's lemma, and
it is dense in `U`. -/
theorem IsTopologicallyFinitelyGenerated.of_openSubgroup_of_finiteIndex
  ...
  exact isTopologicallyFinitelyGenerated_of_fg.of_denseRange
    ...
    (Subgroup.denseRange_subgroupOf_codRestrict hD U.isOpen)
