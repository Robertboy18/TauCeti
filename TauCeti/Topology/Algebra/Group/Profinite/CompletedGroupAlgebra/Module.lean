...
* `TauCeti.IsCompactModule.span_eq_top_of_dense_closure_univ_smul`,
  `TauCeti.IsCompactModule.module_finite_of_dense_closure_univ_smul`: a finite set whose
  `Γ`-orbit generates a dense subgroup spans the module over `R[[Γ]]`, so the module is finitely
  generated.
...
compact. -/
theorem span_eq_top_of_dense_closure_univ_smul {T : Set M} (hT : T.Finite)
    (hgen : Dense (AddSubgroup.closure ((Set.univ : Set Γ) • T) : Set M)) :
    letI := hM.completedGroupAlgebraModule Γ
  ...
  have := hM.isTopologicalAddGroup
  have := hM.t2Space
  have hclosed : IsClosed (Submodule.span (completedGroupAlgebra R Γ) T : Set M) :=
    (Submodule.fg_span hT).isCompact.isClosed
  have hle : AddSubgroup.closure ((Set.univ : Set Γ) • T) ≤
      (Submodule.span (completedGroupAlgebra R Γ) T).toAddSubgroup := by
    rw [AddSubgroup.closure_le]
    intro x hx
    obtain ⟨γ, -, t, ht, rfl⟩ := Set.mem_smul.1 hx
    rw [SetLike.mem_coe, Submodule.mem_toAddSubgroup, ← hM.completedSMul_of γ t,
      ← hM.completedGroupAlgebraModule_smul]
    exact Submodule.smul_mem _ _ (Submodule.subset_span ht)
  have hle' : (AddSubgroup.closure ((Set.univ : Set Γ) • T) : Set M) ⊆
      Submodule.span (completedGroupAlgebra R Γ) T := by
    rw [← Submodule.coe_toAddSubgroup]
    exact SetLike.coe_subset_coe.2 hle
  exact top_unique fun x _ ↦ closure_minimal hle' hclosed (hgen x)

variable (Γ) in
/-- **A compact module in which the `Γ`-orbit of a finite set generates a dense subgroup is a
finitely generated `R[[Γ]]`-module.** -/
theorem module_finite_of_dense_closure_univ_smul {T : Set M} (hT : T.Finite)
    (hgen : Dense (AddSubgroup.closure ((Set.univ : Set Γ) • T) : Set M)) :
    letI := hM.completedGroupAlgebraModule Γ
    Module.Finite (completedGroupAlgebra R Γ) M :=
  letI := hM.completedGroupAlgebraModule Γ
  Module.finite_def.2 (hM.span_eq_top_of_dense_closure_univ_smul Γ hT hgen ▸ Submodule.fg_span hT)
