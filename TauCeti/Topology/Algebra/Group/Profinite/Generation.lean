...
* `TauCeti.topologicalClosure_closure_eq_top_iff`: the same, for the subgroup generated by a
  subset.
* `TauCeti.topologicalClosure_closure_sup_eq_top_iff`: a subset together with a closed normal
  subgroup `K` topologically generates a compact group exactly when its image topologically
  generates `G ⧸ K`.
...
/-- **Generation modulo a closed normal subgroup.** In a compact group, a subset `s` together with
a closed normal subgroup `K` topologically generates the group exactly when the image of `s`
topologically generates the quotient `G ⧸ K`. -/
theorem topologicalClosure_closure_sup_eq_top_iff {s : Set G} {K : Subgroup G} [K.Normal]
  ...
  -- Closedness of `K` makes `G ⧸ K` Hausdorff, so images of compact subgroups are closed.
  have := hK
  set q : G →* G ⧸ K := QuotientGroup.mk' K
  have hq : Continuous q := QuotientGroup.continuous_mk
  have hKq : K.map q = ⊥ := (Subgroup.map_eq_bot_iff K).mpr (QuotientGroup.ker_mk' K).ge
  constructor
  · intro h
    have hmap := congrArg (Subgroup.map q) h
    rwa [q.map_topologicalClosure hq _ (Subgroup.isClosed_topologicalClosure _).isCompact,
      Subgroup.map_sup, MonoidHom.map_closure, hKq, sup_bot_eq,
      Subgroup.map_top_of_surjective q (QuotientGroup.mk'_surjective K)] at hmap
  · intro h
    set L := (Subgroup.closure s ⊔ K).topologicalClosure
    -- The image of `L` is closed and contains the image of `s`, so it is everything.
    have hL : L.map q = ⊤ := by
      refine top_unique (h ▸ Subgroup.topologicalClosure_minimal _ ((Subgroup.closure_le _).mpr ?_)
        (Subgroup.isClosed_map (Subgroup.isClosed_topologicalClosure _).isCompact q hq))
      rintro _ ⟨x, hx, rfl⟩
      exact Subgroup.mem_map_of_mem q
        (Subgroup.le_topologicalClosure _ (Subgroup.mem_sup_left (Subgroup.subset_closure hx)))
    have hcomap := congrArg (Subgroup.comap q) hL
    rwa [Subgroup.comap_map_eq, QuotientGroup.ker_mk', Subgroup.comap_top,
      sup_eq_left.mpr (le_sup_right.trans (Subgroup.le_topologicalClosure _))] at hcomap
