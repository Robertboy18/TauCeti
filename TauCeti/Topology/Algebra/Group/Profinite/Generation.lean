...
* `TauCeti.topologicalClosure_closure_sup_eq_top_iff`: a subset together with a normal subgroup
  `K` topologically generates a topological group exactly when its image topologically generates
  `G ⧸ K`.
...
/-- **Generation modulo a normal subgroup.** In a topological group, a subset `s` together with a
normal subgroup `K` topologically generates the group exactly when the image of `s` topologically
generates the quotient `G ⧸ K`. Neither compactness of `G` nor closedness of `K` is needed: the
quotient map is open, so it exchanges preimages and closures. -/
