...
particular the least number of generators of `R` as a closed normal subgroup of `F` is the
topological generator rank of `R ⧸ Rᵖ[R, F]`.
...
* `TauCeti.IsProP.exists_finset_card_le_topologicalClosure_normalClosure_eq_iff`,
  ...
  generated by `n` elements; so the least number of generators of `R` as a closed normal subgroup
  is the topological generator rank of `R ⧸ Rᵖ[R, F]`.
...
theorem topologicalClosure_normalClosure_eq_iff_quotient_pLowerCentralStep (s : Set R) :
  ...
  have hs : Subtype.val '' s ⊆ R := by
    rintro _ ⟨x, -, rfl⟩
    exact x.2
  have hclosed : IsClosed ((pLowerCentralStep p R).subgroupOf R : Set R) := by
    rw [coe_subgroupOf, coe_subtype]
    exact (isClosed_pLowerCentralStep R).preimage continuous_subtype_val
  rw [hF.topologicalClosure_normalClosure_eq_iff_topologicalClosure_sup_eq hp hR hs,
    ← topologicalClosure_closure_sup_eq_top_iff hclosed]
