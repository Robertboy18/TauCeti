...
with `ℤ_p ^ r × T` for a finite abelian `p`-group `T`. This file adds the uniqueness clauses of
that theorem and the description of its two factors.
...
* The invariants are unique: in any two decompositions `A ≅ ℤ_p ^ r × T ≅ ℤ_p ^ r' × T'` with
  `T` and `T'` torsion, `r = r'` and `T ≅ T'`. The rank is a topological invariant because a
  continuous additive isomorphism `ℤ_p ^ r ≃ ℤ_p ^ r'` is `ℤ_p`-linear, and the torsion factors are
  both the torsion subgroup of `A`.
...
* `TauCeti.eq_of_continuousMulEquiv_prod_padicInt`: uniqueness of the rank `r`.
...
theorem eq_of_continuousMulEquiv_prod_padicInt {r r' : ℕ} {T T' : Type*} [AddCommGroup T]
...
theorem finite_torsion (hA : IsProP p A) (hfg : IsTopologicallyFinitelyGenerated A) :
  ...
  exact Finite.of_equiv _
    (torsionMulEquivOfMulEquiv isAddTorsion_of_finite f.toMulEquiv).symm.toEquiv
...
theorem isOpen_torsion_iff_finite (hA : IsProP p A) (hfg : IsTopologicallyFinitelyGenerated A) :
  ...
  exact Finite.of_equiv _ (Subgroup.groupEquivQuotientProdSubgroup (s := torsion A)).symm
...
theorem exists_continuousMulEquiv_pi_padicInt [IsMulTorsionFree A] (hA : IsProP p A)
  ...
  refine ⟨r, ⟨f.trans
    { toFun x := ofAdd x.toAdd.1
      invFun v := ofAdd (v.toAdd, 0)
      left_inv x := by
        simp only [toAdd_ofAdd]
        rw [← ht x.toAdd.2, Prod.mk.eta, ofAdd_toAdd]
      right_inv v := by simp
      map_mul' x y := by simp
      continuous_toFun := continuous_ofAdd.comp (continuous_fst.comp continuous_toAdd)
      continuous_invFun := continuous_ofAdd.comp (continuous_toAdd.prodMk continuous_const) }⟩⟩
