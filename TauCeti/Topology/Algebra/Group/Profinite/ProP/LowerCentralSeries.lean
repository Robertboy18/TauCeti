...
`Gᵖ [G, G]`; for a profinite `G` and a prime `p` that is the pro-`p` Frattini subgroup
`TauCeti.proPFrattini p G`. One step of the recursion is `TauCeti.pLowerCentralStep`, whose
characteristic property `TauCeti.pLowerCentralStep_le_iff` is what every statement below is
reduced to.
...
For a topologically finitely generated profinite group every `λ_k` is open, hence of finite index:
`λ_k` is itself a topologically finitely generated profinite group, so its pro-`p` Frattini
subgroup is open in it, and that Frattini subgroup lies in `λ_{k+1}`. In a pro-`p` group the
quotients `G ⧸ λ_k` are then finite `p`-groups. Without finite generation the terms need not be
open: an infinite product of copies of `ℤ ⧸ p` has `λ_1 = 1`.
...
* `TauCeti.pLowerCentralSeries_one_eq_proPFrattini`: `λ_1` is the pro-`p` Frattini subgroup of a
  profinite group.
* `MonoidHom.map_pLowerCentralSeries_le`, `TauCeti.map_pLowerCentralSeries_eq_of_surjective`,
  `ContinuousMulEquiv.map_pLowerCentralSeries_eq`: functoriality of the series.
* `TauCeti.IsTopologicallyFinitelyGenerated.isOpen_pLowerCentralSeries`: in a topologically
  finitely generated profinite group every `λ_k` is open, so
...
/-- The defining equation of `TauCeti.pLowerCentralStep`: its body is not exposed, so unfolding it
...
theorem map_pLowerCentralStep_eq_of_surjective [CompactSpace G] [T2Space H] (f : G →* H)
    (hf : Continuous f) (hsurj : Function.Surjective f) (K : Subgroup G) :
...
(`TauCeti.commutator_pLowerCentralSeries_le`), and `λ_1` is the pro-`p` Frattini subgroup of a
profinite group (`TauCeti.pLowerCentralSeries_one_eq_proPFrattini`). -/
...
theorem commutator_pLowerCentralSeries_le (j k : ℕ) :
  ...
  | succ k ih =>
    ...
    refine sup_le ?_ ?_
    · -- The `p`-th powers: `⁅a, y ^ p⁆ = ⁅a, y⁆ ^ p` modulo `N`, because `⁅a, y⁆ ∈ λ_m` is central
      -- there, and `⁅a, y⁆ ^ p ∈ N`.
      ...
      have hcentral : QuotientGroup.mk' N ⁅a, y⁆ ∈ center (G ⧸ N) :=
        map_mk'_pLowerCentralSeries_le_center (j + k + 1) (mem_map_of_mem _ hc)
      have hcomm :
          Commute (QuotientGroup.mk' N y) ⁅QuotientGroup.mk' N a, QuotientGroup.mk' N y⁆ := by
        rw [← map_commutatorElement]
        exact mem_center_iff.mp hcentral _
      rw [← QuotientGroup.eq_one_iff (N := N), ← QuotientGroup.mk'_apply, map_commutatorElement,
        map_pow, ← hcomm.commutatorElement_pow_right, ← map_commutatorElement, ← map_pow,
        QuotientGroup.mk'_apply, QuotientGroup.eq_one_iff]
      exact pow_mem_pLowerCentralSeries hc
    · -- The commutators: the three subgroups lemma in `G ⧸ N`.
      ...
      · rw [← map_commutator, ← commutator_le_iff_map_mk'_eq_bot]
        calc ⁅⁅(⊤ : Subgroup G), pLowerCentralSeries p G j⁆, pLowerCentralSeries p G k⁆
          ...
          _ = N := by rw [hN, show j + 1 + k + 1 = j + k + 1 + 1 by omega]
      · rw [← map_commutator, ← commutator_le_iff_map_mk'_eq_bot]
...
`λ_k`. -/
theorem map_pLowerCentralSeries_eq_of_surjective [CompactSpace G] [T2Space H] (f : G →* H)
    (hf : Continuous f) (hsurj : Function.Surjective f) (k : ℕ) :
    (pLowerCentralSeries p G k).map f = pLowerCentralSeries p H k := by
  induction k with
  | zero =>
  ...
  | succ k ih =>
    rw [pLowerCentralSeries_succ, pLowerCentralSeries_succ,
      map_pLowerCentralStep_eq_of_surjective f hf hsurj, ih]
...
/-- **Openness of the lower `p`-series.** In a topologically finitely generated profinite group
every term of the lower `p`-series is open. Each `λ_k` is itself a topologically finitely generated
profinite group, so its pro-`p` Frattini subgroup is open in it, and that subgroup lies in
`λ_{k+1}`. -/
...
/-- In a topologically finitely generated profinite group every quotient `G ⧸ λ_k` is finite. -/
...
/-- In a topologically finitely generated pro-`p` group every quotient `G ⧸ λ_k` is a finite
`p`-group. -/
