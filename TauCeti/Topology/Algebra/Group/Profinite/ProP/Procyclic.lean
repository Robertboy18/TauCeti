...
`u` is a unit of `ℤ_[p]`. If `u` is a unit then `a = (a ^ u) ^ (u⁻¹)` lies in the closed subgroup
generated by `a ^ u`; if `u = p * w` is not a unit then `a ^ u = (a ^ w) ^ p` lies in the pro-`p`
Frattini subgroup, which is closed and proper. Consequently any two topological generators of a
nontrivial procyclic pro-`p` group differ by a unit exponent.
