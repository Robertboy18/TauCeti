...
The finite powers `ℤ_[p] ^ X` are treated at the end: they are pro-`p`, topologically generated by
the coordinate vectors, so that a continuous homomorphism out of `Multiplicative (X → ℤ_[p])` is
determined by its values on them, and the `p`-adic power by `l` is scalar multiplication by `l`,
that is coordinatewise multiplication by `l`.
...
* `TauCeti.isProP_multiplicative_pi_padicInt`,
  ...
  topologically generated by the coordinate vectors for finite `X`, and continuous homomorphisms
  out of it are determined by their values on them.
...
/-- The coordinate vectors topologically generate the additive group `ℤ_[p] ^ X`, because the
integer vectors are dense. -/
theorem topologicallyGenerates_ofAdd_single_padicInt :
  ...
  -- Every integer vector lies in the subgroup generated by the coordinate vectors.
  have hint : ∀ n : X → ℤ, ofAdd (fun x ↦ (n x : ℤ_[p])) ∈ S := by
    intro n
    have h : (fun x ↦ (n x : ℤ_[p])) = ∑ x, n x • Pi.single x (1 : ℤ_[p]) := by
      ext y
      simp [Pi.single_apply]
    rw [h, ofAdd_sum]
    refine Subgroup.prod_mem _ fun x _ ↦ ?_
    rw [ofAdd_zsmul]
    exact Subgroup.zpow_mem _ (Subgroup.subset_closure (Set.mem_range_self x)) _
  -- The integer vectors are dense in `ℤ_[p] ^ X`.
  have hdense : Dense (S : Set (Multiplicative (X → ℤ_[p]))) := by
    have h1 : DenseRange (Pi.map fun _ : X ↦ (Int.cast : ℤ → ℤ_[p])) :=
      DenseRange.piMap fun _ ↦ PadicInt.denseRange_intCast
    have h2 : DenseRange fun n : X → ℤ ↦ ofAdd (fun x ↦ (n x : ℤ_[p])) :=
      ofAdd.surjective.denseRange.comp h1 continuous_ofAdd
    exact h2.mono (Set.range_subset_iff.mpr hint)
  exact SetLike.coe_injective (by
    rw [Subgroup.topologicalClosure_coe, hdense.closure_eq, Subgroup.coe_top])
