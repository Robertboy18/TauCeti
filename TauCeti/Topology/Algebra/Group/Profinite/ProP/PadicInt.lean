...
range of its `p`-adic power map** `l ↦ a ^ l`: the range is compact, hence closed, and contains
`a`, while every `p`-adic power of `a` lies in every closed subgroup containing `a`. -/
