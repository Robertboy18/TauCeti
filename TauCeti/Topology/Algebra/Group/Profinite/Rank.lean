...
`d(U) ≤ 1 + [G : U] * (d(G) - 1)`, with subtraction in `ℕ`. This is Schreier's index formula
`Subgroup.rank_le_one_add_index_mul_rank_sub_one` applied to a dense subgroup `D` of `G`
generated by `d(G)` elements: `U ⊓ D` has index at most `[G : U]` in `D`, and it is dense in
`U`. The bound is sharp: the open subgroups of a free group attain it, by the Nielsen–Schreier
theorem. -/
