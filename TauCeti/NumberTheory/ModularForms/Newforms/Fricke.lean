...
theorem commute_normalizedFrickeCharCuspOneEnd_heckeRingHomCuspCharSpace {n : ℕ}
  ...
  -- the Hecke ring elements whose action commutes with `𝒲_N` form a subring, containing every
  -- double coset of determinant prime to `N`
  have hS : Subring.closure {x : 𝕋 (Delta0 N) ((Gamma0 N).map (mapGL ℚ)) ℤ |
      ∃ D, CoprimeDetCoset N N D ∧ x = HeckeCosetModule.single ℤ D 1} ≤
        (Subring.centralizer {normalizedFrickeCharCuspOneEnd (N := N) k}).comap
  ...
  -- and the good `Tₙ` lie in the subring those cosets generate
  simpa [Subring.mem_centralizer_iff, Set.mem_centralizer_iff, Commute, SemiconjBy] using
