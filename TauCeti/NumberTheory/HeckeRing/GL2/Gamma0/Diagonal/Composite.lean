...
coefficients in the classes of double cosets `Γ₀(N) α Γ₀(N)` with `det α` prime to `m`: the
generator `T_p` at a prime `p ∤ m` is the class of `diag(1, p)`, the scalar coset `T(p, p)` is the
class of `diag(p, p)` when `p` is prime to the level and `0` otherwise, the prime-power blocks are
built from them by the Diamond–Shurman recurrence, and the composite is the product of the blocks.
...
theorem heckeTCompositeGamma0_mem_closure_coprimeDetCoset {m n : ℕ} (hn : n.Coprime m) :
  ...
  -- hence every `T_{p^v}` at a prime `p ∤ m`, by the recurrence
  ...
  -- and every `T_n` with `n` prime to `m`, by multiplicativity
