...
/-!
# Submodules invariant under a group action

Let a group `Γ` act on an `R`-module `M` by `R`-linear maps, that is by a `DistribMulAction Γ M`
...
For a compact group acting continuously on a topological module the invariant cores of the open
submodules are open, so the invariant open submodules form a basis of neighbourhoods of zero;
that is where the invariant core is used.
...
* `Submodule.quotientToModuleEnd_mk`, `Submodule.mem_ker_quotientToModuleEnd`,
  `Submodule.ker_quotientToModuleEnd_mono`, `Submodule.factor_quotientToModuleEnd`: the
...
variable {Γ R M : Type*} [Group Γ] [Semiring R] [AddCommMonoid M] [Module R M]
  [DistribMulAction Γ M] [SMulCommClass Γ R M]

variable (Γ) in
/-- The **invariant core** of a submodule `V` under the action of a group `Γ`: the elements `x`
...
variable {Γ R M : Type*} [Group Γ] [Ring R] [AddCommGroup M] [Module R M]
...
theorem mem_ker_quotientToModuleEnd (hV : ∀ γ : Γ, ∀ x ∈ V, γ • x ∈ V) {γ : Γ} :
    γ ∈ (quotientToModuleEnd hV).ker ↔ ∀ x : M, γ • x - x ∈ V := by
  simp only [MonoidHom.mem_ker, LinearMap.ext_iff, Module.End.one_apply]
...
theorem ker_quotientToModuleEnd_mono (hV : ∀ γ : Γ, ∀ x ∈ V, γ • x ∈ V)
    (hV' : ∀ γ : Γ, ∀ x ∈ V', γ • x ∈ V') (h : V ≤ V') :
    (quotientToModuleEnd hV).ker ≤ (quotientToModuleEnd hV').ker := fun γ hγ => by
  rw [mem_ker_quotientToModuleEnd] at hγ ⊢
  exact fun x => h (hγ x)
