...
The subgroup generated by a set that is stable under conjugation is normal: conjugation by a fixed
element is a group homomorphism, so it preserves the subgroup generated by any set it preserves.
...
theorem _root_.Subgroup.closure_normal_of_forall_conj_mem {S : Set G}
    (h : ∀ g : G, ∀ s ∈ S, g * s * g⁻¹ ∈ S) : (closure S).Normal where
  conj_mem n hn g := by
    induction hn using closure_induction with
    | mem s hs => exact subset_closure (h g s hs)
    | one => simp
    | mul a b _ _ ha hb =>
      have : g * (a * b) * g⁻¹ = g * a * g⁻¹ * (g * b * g⁻¹) := by group
      rw [this]
      exact mul_mem ha hb
    | inv a _ ha =>
      have : g * a⁻¹ * g⁻¹ = (g * a * g⁻¹)⁻¹ := by group
      rw [this]
      exact inv_mem ha
