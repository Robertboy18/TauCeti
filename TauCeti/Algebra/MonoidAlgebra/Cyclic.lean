...
the finite cyclic group `C`: its coefficients are the coordinates of `f(σ - 1)` in the basis of
powers of `σ - 1`. -/
...
theorem aeval_of_sub_one_one_add_X_pow_card_sub_one (σ : C) :
...
group `C`: its range contains the spanning set of powers of `σ - 1`. -/
...
is the principal ideal generated by `(1 + X) ^ |C| - 1`: a polynomial vanishing at `σ - 1` has a
remainder of degree `< |C|` modulo this monic polynomial which still vanishes at `σ - 1`, hence is
zero. -/
theorem ker_aeval_of_sub_one (hσ : ∀ x, x ∈ Subgroup.zpowers σ) :
  ...
  refine ⟨fun hf ↦ ?_, fun ⟨q, hq⟩ ↦ by
    rw [hq, map_mul, aeval_of_sub_one_one_add_X_pow_card_sub_one, zero_mul]⟩
  rcases subsingleton_or_nontrivial R with hR | hR
  ...
  have hr : aeval (MonoidAlgebra.of R C σ - 1) (f %ₘ ((1 + X) ^ Nat.card C - 1)) = 0 := by
    rw [modByMonic_eq_sub_mul_div f, map_sub, map_mul, hf,
      aeval_of_sub_one_one_add_X_pow_card_sub_one, zero_mul, sub_zero]
