...
* `CompleteLattice.IsCompactElement.map` and `OrderIso.isCompactElement_iff`: an order
  isomorphism of complete lattices preserves and reflects compactness of elements.
-/

public section

/-- An order isomorphism of complete lattices sends compact elements to compact elements. -/
theorem CompleteLattice.IsCompactElement.map {α β : Type*} [CompleteLattice α]
    [CompleteLattice β] (f : α ≃o β) {a : α} (ha : IsCompactElement a) :
    IsCompactElement (f a) := by
...
theorem OrderIso.isCompactElement_iff {α β : Type*} [CompleteLattice α] [CompleteLattice β]
  ...
  ⟨fun h ↦ by simpa using CompleteLattice.IsCompactElement.map f.symm h,
    CompleteLattice.IsCompactElement.map f⟩
