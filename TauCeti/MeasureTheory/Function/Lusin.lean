...
* `TauCeti.exists_isClosed_measure_diff_lt_continuousOn` — **Lusin's theorem**: a measurable map
...
* `TauCeti.exists_isClosed_measure_diff_lt_continuousOn_of_aemeasurable` — the same for an
...
theorem exists_isClosed_measure_diff_lt_continuousOn (hf : Measurable f) (hs : MeasurableSet s)
    (hμs : μ s ≠ ∞) (hε : ε ≠ 0) :
...
`ε > 0`. -/
theorem exists_isClosed_measure_diff_lt_continuousOn_of_aemeasurable (hf : AEMeasurable f μ)
  ...
  obtain ⟨F, hFs, hF, hFμ, hFf⟩ := exists_isClosed_measure_diff_lt_continuousOn hf.measurable_mk
