...
`1` on every standard basis vector, so it is alternating only when the index type is empty. Since
the identity matrix is symmetric and invertible, the standard form is in every positive dimension
a nondegenerate symmetric form that is not alternating, the model for the orthonormal normal form
of such forms.

## Main results

* `Matrix.isAlt_toBilin'_one_iff`: the standard form on `n → R` is alternating exactly when `n`
  is empty.
-/
