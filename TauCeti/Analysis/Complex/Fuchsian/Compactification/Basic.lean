...
Its topology is the usual one: a set is open when its trace on `Γ \ ℍ` is open and, for every
cusp orbit it contains and every normalized cusp datum `D` representing that orbit, it contains
the image of a sufficiently high horodisc at `D` together with the cusp orbit. The basic
...
heights (`TauCeti.Subgroup.CuspDatum.exists_image_quotientMk_horodisc_eq`).

The coarse quotient embeds as an open subset (`isOpenEmbedding_ofQuotient`), whose complement is
the closed set of cusp orbits. For a discrete `Γ` the coarse quotient is dense, and the
compactified quotient is Hausdorff and second countable: two cusps are separated by high horodiscs
...
* `Subgroup.CompactifiedQuotient`: the carrier, with its topology.
...
* `Subgroup.CompactifiedQuotient.instT2Space` and
  `Subgroup.CompactifiedQuotient.instSecondCountableTopology`: for a discrete `Γ`, the
  compactified quotient is Hausdorff and second countable.
...
neighbourhood at every cusp datum whose cusp orbit it contains. -/
instance : TopologicalSpace Γ.CompactifiedQuotient where
  IsOpen U := IsOpen (ofQuotient ⁻¹' U) ∧
    ∀ D : Γ.CuspDatum, ofCusp D.cuspOrbit ∈ U → ∃ A : ℝ, cuspNhd D A ⊆ U
  isOpen_univ := ⟨isOpen_univ, fun _ _ ↦ ⟨0, subset_univ _⟩⟩
  isOpen_inter U V hU hV := ⟨hU.1.inter hV.1, fun D hD ↦ by
    obtain ⟨A, hA⟩ := hU.2 D hD.1
    obtain ⟨B, hB⟩ := hV.2 D hD.2
    exact ⟨max A B, subset_inter ((cuspNhd_antitone D (le_max_left A B)).trans hA)
      ((cuspNhd_antitone D (le_max_right A B)).trans hB)⟩⟩
  isOpen_sUnion S hS := ⟨by
    rw [preimage_sUnion]
    exact isOpen_biUnion fun U hU ↦ (hS U hU).1, fun D hD ↦ by
    obtain ⟨U, hU, hDU⟩ := mem_sUnion.mp hD
    obtain ⟨A, hA⟩ := (hS U hU).2 D hDU
    exact ⟨A, hA.trans (subset_sUnion_of_mem hU)⟩⟩
...
theorem isOpenMap_ofQuotient : IsOpenMap (ofQuotient (Γ := Γ)) := fun V hV ↦
  ⟨by rwa [ofQuotient_injective.preimage_image], fun D hD ↦ by
    obtain ⟨p, -, hp⟩ := hD
    cases hp⟩
...
theorem isOpen_cuspNhd : IsOpen (cuspNhd D A) := by
  ...
  · obtain ⟨a, -, h⟩ := exists_cuspNhd_eq D ((ofCusp_mem_cuspNhd_iff D A).mp hD')
    exact ⟨a * A, (h A).le⟩
...
theorem nhds_basis_cuspNhd (A₀ : ℝ) :
  ...
  obtain ⟨A, hA⟩ := hV.2 D hxV
  exact ⟨max A₀ A, le_max_left _ _, (cuspNhd_antitone D (le_max_right A₀ A)).trans (hA.trans hVU)⟩
...
/-! ### Separation and countability for discrete groups -/

variable [DiscreteTopology Γ]

/-- **The coarse orbit space is dense in the compactified quotient** of a discrete group. -/
...
/-- **The compactified quotient of a discrete group is Hausdorff.** -/
...
/-- **The compactified quotient of a discrete group is second countable**: the coarse orbit space
is second countable, there are countably many cusp orbits, and each has the countable
neighbourhood basis of cusp neighbourhoods at integer heights. -/
